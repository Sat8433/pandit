...
<head>
    ...
    <title>Edit Pooja - Admin Dashboard</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Pooja: {{ $pooja->name }}</h1>
        </div>

        <form action="{{ route('admin.poojas.update', $pooja->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $pooja->name }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    ...
                    <select name="category" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                        <option value="general" {{ $pooja->category == 'general' ? 'selected' : '' }}>General</option>
                        <option value="ceremony" {{ $pooja->category == 'ceremony' ? 'selected' : '' }}>Ceremony</option>
                        <option value="festival" {{ $pooja->category == 'festival' ? 'selected' : '' }}>Festival</option>
                        <option value="vedic" {{ $pooja->category == 'vedic' ? 'selected' : '' }}>Vedic</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Base Price (₹) <span class="text-red-500">*</span></label>
                    <input type="number" step="0.01" name="base_price" value="{{ $pooja->base_price }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Duration (minutes) <span class="text-red-500">*</span></label>
                    <input type="number" name="duration_minutes" value="{{ $pooja->duration_minutes }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-500">*</span></label>
                <textarea name="description" rows="4" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">{{ $pooja->description }}</textarea>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-md bg-gray-50">
                    @forelse($samagris as $samagri)
                        <label class="flex items-center space-x-3">
                            <input type="checkbox" name="samagri[]" value="{{ $samagri->id }}" 
                                {{ $pooja->samagriItems->contains($samagri->id) ? 'checked' : '' }} 
            ...
            <div class="flex justify-between pt-4 border-t">
                <div>
                   <label class="inline-flex items-center">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ $pooja->is_active ? 'checked' : '' }} class="rounded text-purple-600 focus:ring-purple-500">
                        <span class="ml-2 text-sm text-gray-700 font-medium">Pooja is Active</span>
                   </label>
                </div>
                <button type="submit" class="bg-purple-600 text-white px-6 py-2 rounded-md font-medium hover:bg-purple-700">Update Pooja</button>
            </div>
        </form>
    </div>
</body>
